...
class Focus(nn.Module):
    ...
    def forward(self, x):  # x(b,c,w,h) -> y(b,4c,w/2,h/2) -> cut into 4 scuares and stack them
...
class NMS(nn.Module):
    ...
    def forward(self, x):
        return non_max_suppression(x[0], conf_thres=self.conf, iou_thres=self.iou, classes=self.classes)  # why x[0]?


class autoShape(nn.Module):  # TODO:
